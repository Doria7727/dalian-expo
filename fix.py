...
# 验证
for path, must_have, must_not in [
    ("index.html", "2027大连国际工业博览会", "中国国际工业装备博览会 - 官网首页"),
    ("assets/js/main.js", "brand-logo", "hero-top"),
    ("assets/css/style.css", "brand-logo", "hero-bg-real.jpg"),
]:
    with open(path, "r", encoding="utf-8") as f: c = f.read()
    assert must_have in c, f"{path} missing {must_have}"
    assert must_not not in c, f"{path} still has {must_not}"
